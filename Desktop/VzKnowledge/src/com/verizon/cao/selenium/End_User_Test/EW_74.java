...
public class EW_74 extends SetupBase implements Constants {
	...
	public void setUp() throws Exception {
		...
		Log.startTestCase("News_Authoring");
	...
	public void create_news() throws Exception {
		test = extent.startTest("News_Authoring");
		...
		if (cmf.existsElement(EndUser.clk_authoring, driver) == true) {
			Log.info("Authoring Exist in BG " + Select_Bg);
			test.log(LogStatus.PASS, "Authoring", "Authoring Exist in BG " + Select_Bg);
		} else {
			Log.info("Authoring not Exist in BG " + Select_Bg);
			test.log(LogStatus.INFO, "Authoring", "Authoring not Exist in BG " + Select_Bg);
			return;
		}
		...
		cmf.SelectDropDown(driver, EndUser.auth_set_bgroup, Select_Bg, extent, test);
		...
		cmf.clkElement(driver, "//input[@title='" + Role_access + "']", "Select Role", extent, test);
		...
		cmf.send_data(driver, EndUser.authoring_news_desc, News_desc, extent, test);
		...
		//cmf.send_data(driver, EndUser.authoring_body_desc, News_message, extent, test);
