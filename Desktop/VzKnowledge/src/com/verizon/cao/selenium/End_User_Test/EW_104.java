...
public class EW_104 extends SetupBase implements Constants {
	...
	public EW_104() throws Exception {
		super();
	}
	...
	public void setUp() throws Exception {
		...
		Log.startTestCase("NetworkBG_My_Bookmark");
	...
	public void my_bookmark() throws Exception {
		test = extent.startTest("NetworkBG_My_Bookmark");
		...
		cmf.clkElement(driver, EndUser.clk_first_tax_topic, "Taxonomy", extent, test);
			...
			cmf.mouse_click(driver, EndUser.clk_first_taxonomy, "Taxonomy", extent, test);
		...
		cmf.sleep(3000);
		try {
			cmf.mouse_ie(driver, EndUser.my_book_mark, "My Bookmark", extent, test);
		...
		if (cmf.existsElement(EndUser.my_bookmark_search + "'Test"+CommonFunctions.timeStamp+"')]", driver) == true) {
			...
			try {
				...
				Ext_Report.CaptureScreen(driver, Screen_PATH+"EW_104");
				Log.info("Exception occured while search bookmark title " + e);
				test.log(LogStatus.FAIL, "Bookamrk", "Exception occured while search bookmark title " + e);
				throw new Exception("Bookmark title" + get_con_title + " failed to click " + e);
			}

		} else {
			String get_screen_path = Ext_Report.CaptureScreen(driver, Screen_PATH+"EW_104");
			Log.info("Bookmarks are not avilable");
			test.log(LogStatus.FAIL, "Bookamrk", "Bookmark Title " +get_con_title+" not avialble while navigate to 'My Bookmark' "+test.addScreenCapture(Report_Screen_PATH+"EW_104.png"));
