...
public class EW_57 extends SetupBase implements Constants {
	...
	public void setUp() throws Exception {
		...
		Log.startTestCase("Admin_Feedback_Inbox");
...
//		System.out.println("inside before class" + baseurl);
	...
	public void feedback_inbox() throws Exception {
		test = extent.startTest("Admin_Feedback_Inbox");
		...
		if (driver.findElement(By.xpath(EndUser.chk_feedback_cmt)).getText().contains(admin_feedback_cmt)) {
			...
			cmf.send_data(driver, EndUser.feedback_response, feedback_admin_response, extent, test);
			cmf.sleep(1000);
			...
			if (cmf.existsElement(EndUser.message_total_page, driver) == true) {
				navigate_next(driver,EndUser.message_total_page, EndUser.message_inbox_next,
						EndUser.author_feedback_cmt + feedback_admin_response + "']" + EndUser.feedback_action_detail);
				...
				Ext_Report.CaptureScreen(driver, Screen_PATH+"EW_57");
				Log.info("Replied Feedback is not listed with the Action 'View Details'");
				test.log(LogStatus.FAIL, "Feedback", "Replied Feedback is not listed with the Action 'View Details "+test.addScreenCapture(Report_Screen_PATH+"EW_57.png"));
			...
			test.log(LogStatus.FAIL, "Feedback Comment", "Newly added feedback comment is not listed in the top most");
