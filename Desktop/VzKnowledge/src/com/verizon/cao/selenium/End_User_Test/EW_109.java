...
public class EW_109 extends SetupBase implements Constants {
	...
	public EW_109() throws Exception {
		super();
	}
	...
	public void setUp() throws Exception {
		...
		Log.startTestCase("NetworkBG_Trending");
	...
	public void trending() {
		test = extent.startTest("NetworkBG_Trending");
			...
			if (cmf.existsElement(EndUser.chk_trending_section, driver) == true) {
				...
				if (cmf.existsElement(EndUser.chk_all_article + "[" + count + "]" + "//h2", driver) == true) {
					...
					if (cmf.existsElement(EndUser.chk_all_article + "[" + count + "]" + "//li", driver) == true) {
						...
						Log.info("Trending Section " + trending_section[trend] + " is appeared with items in the BG "
								+ Network_Bg);
						test.log(LogStatus.PASS, trending_section[trend], "Trending Section " + trending_section[trend]
								+ " is appeared with items in the BG " + Network_Bg);
					} else {
						System.out
								.println("Trending Section " + trending_section[trend] + " is not appeared with items");
						Log.info("Trending Section " + trending_section[trend]
								+ " is not appeared with items in the BG " + Network_Bg);
						test.log(LogStatus.INFO, trending_section[trend], "Trending Section " + trending_section[trend]
								+ " is not appeared with items in the BG " + Network_Bg);
					}
				} else {
					System.out.println("Trending Section " + trending_section[trend] + " is not appeared");
					Log.info("Trending Section " + trending_section[trend] + " is not appear in the BG " + Network_Bg);
					test.log(LogStatus.INFO, trending_section[trend],
							"Trending Section " + trending_section[trend] + " is not appear in the BG " + Network_Bg);
				}
			} else {
				System.out.println("Trending article section not found");
				Log.info("Trending article section not found in the BG " + Network_Bg);
				test.log(LogStatus.INFO, "Trending", "Trending article section not found in the BG " + Network_Bg);
			}
		}
	}
