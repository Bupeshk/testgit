...
public class EW_51 extends SetupBase implements Constants {
	...
	public void setUp() throws Exception {
		...
		Log.startTestCase("Login Validation User");
	...
	public void chk_left_nav() {
		...
		test = extent.startTest("Login Validation User");
		WebDriver driver = CommonFunctions.driver;
		if (! CommonFunctions.executeTestCase){
			test.log(LogStatus.SKIP, "Skipping the test case as error encountered during login.", "Usage: <span style='font-weight:bold;'>Skipped test cases.</span> Login Validation User.");
...
/*		SignInPage.UserID(driver, extent, test).sendKeys(Username);
		...
		if (get_content.size() != 0) {
			...
			test.log(LogStatus.INFO, "Content Types", "Content types are not available in the business group " + Select_Bg);
		}
		...
		if (get_tool.size() != 0) {
			...
			//String[] chk_tool = {"Admin", "Authoring", "Calendar", "Communication Hub", "Contacts", "eGlossary", "FAQ", "News"};
			for (WebElement we : get_tool) {
				System.out.println(we.getText());
				Log.info("Tool type "+we.getText()+" is display in the business group " + Select_Bg);
				test.log(LogStatus.INFO, "Tool Type", "Tool type "+we.getText()+" is display in the business group " + Select_Bg);
			}
		}
		else{
			Log.info("Tools types are not available");
			test.log(LogStatus.FAIL, "Tool Types", "Tools types are not available in the business group " + Select_Bg);
		}
		if (midpage.chk_news(driver, EndUser.chk_news_feed) == true) {
			...
			test.log(LogStatus.INFO, "News", "News Feed is not available in BG " + Select_Bg);
		}
