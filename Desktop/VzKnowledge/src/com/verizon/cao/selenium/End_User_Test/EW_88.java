...
public class EW_88 extends SetupBase implements Constants{
...
public void search() throws Exception{
	...
	cmf.send_data(driver, EndUser.enter_search, basic_search_con, extent, test);
	...
	element = MiddlePage.auto_search(driver, EndUser.clk_search_all, basic_search_con, EndUser.chk_auto_search, extent, test);
	if (element != null){
		//List<WebElement> = element;
		Log.info("Auto Search is display and the count is ");
		test.log(LogStatus.PASS, "Auto Search", "Auto Search is display with 10 items ");
	}
	else{
		Log.info("Auto Search is not display");
		test.log(LogStatus.FAIL, "Auto Search", "Auto Search is not display");
		Reporter.log("Auto Search is not display");
		throw new Exception("Auto Search is not display");
	}
