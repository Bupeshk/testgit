...
public class EW_129 extends SetupBase implements Constants {
	...
	public EW_129() throws Exception {
		super();
	}
	...
	public void contacts() throws Exception {
		test = extent.startTest("Contacts");
		...
		cmf.sleep(1000);
		cmf.send_data(driver, EndUser.contact_name, Contact_name, extent, test);
		cmf.send_data(driver, EndUser.contact_email, Contact_email, extent, test);
		cmf.send_data(driver, EndUser.contact_phone, Contact_phone, extent, test);
		cmf.clkElement(driver, EndUser.sel_contact_category, "Click Category", extent, test);
		cmf.sleep(500);
		WebElement elem = cmf.checkBox(EndUser.clk_contact_category + "'" + Contact_category + "']", driver);
		String get_id = elem.getAttribute("for");
		cmf.clkElement(driver, "//input[@id='" + get_id + "']", "Select", extent, test);
		...
		if (driver.findElement(By.xpath(EndUser.chk_contact_name)).getText().equalsIgnoreCase(Contact_name)) {
			...
			test.log(LogStatus.PASS, "Contact",
					"Newly created conatct appeared at first row and the contact name is  " + Contact_name);
		}
		else{
				System.out.println("Newly created contact "+Contact_name+" is not appeared for the user account "+Username);
				Log.info("Newly created contact "+Contact_name+" is not appeared for the user account "+Username);
				test.log(LogStatus.FAIL, "Contact", "Newly created contact "+Contact_name+" is not appeared for the user account "+Username);
				Reporter.log("Newly created contact "+Contact_name+" is not appeared for the user account "+Username);
				throw new Exception("Newly created contact "+Contact_name+" is not appeared for the user account "+Username);
			}
	}
