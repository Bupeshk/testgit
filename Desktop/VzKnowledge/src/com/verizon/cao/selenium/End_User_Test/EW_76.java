...
public class EW_76 extends SetupBase implements Constants{
	...
	public void setUp() throws Exception {
		...
		Log.startTestCase("News_Validation");
	...
	public void chk_around_verizon(){
		test = extent.startTest("Check_Around_Verizon");
		...
		if (cmf.existsElement(EndUser.right_hand_side+"[text()='Around Verizon']", driver) == true){
			Log.info("Around Verizon Tab is available in the Right Hand Side");
			test.log(LogStatus.INFO, "Around Verizon","Around Verizon Tab is available in the Right Hand Side");
			cmf.sleep(5000);
			//int count = 5;
			for (int i = 0; i < 5; i++){
				if (cmf.existsElement(EndUser.clk_around_nav+i+"']", driver) == true){
					...
					if (elem.getText().contains(News_title)){
						...
						Log.info("Newly cretaed news "+News_title+" is display in the Around verizon");
						test.log(LogStatus.PASS, "Around Verizon","Newly cretaed news "+News_title+" is display in the Around verizon");
						break;
					}
					else if(i == 4){
						Log.info("Newly cretaed news "+News_title+" is not display in the Around verizon");
						test.log(LogStatus.INFO, "Around Verizon","Newly cretaed news "+News_title+" is not display in the Around verizon");
					}
				}
			}
		}
