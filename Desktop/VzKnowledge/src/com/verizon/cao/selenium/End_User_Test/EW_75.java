...
public class EW_75 extends SetupBase implements Constants {
	...
	public void news_validate() {
		test = extent.startTest("News_Validation");
		...
		if (midpage.chk_news(driver, EndUser.chk_news_feed) == true) {
			...
			for (int i = 0; i < scroll_news.size(); i++) {
				...
				if (title_news.equalsIgnoreCase(News_title)) {
					...
					test.log(LogStatus.PASS, "News", "Newly created news is available and the title is " + News_title);
					break;
				} else {
			...
			Log.info("News Feed is not available in BG " + Select_Bg);
