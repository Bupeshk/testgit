...
public class EW_66 extends SetupBase implements Constants {
...
//		System.out.println("inside before class" + baseurl);
	...
	@Test
	public void alert_box() throws Exception {
		test = extent.startTest("Alert");
	...
	/*	SignInPage.UserID(driver, extent, test).sendKeys(Username);
		...
		// String alert_unread =
		// driver.findElement(By.xpath(EndUser.alert_unread_count)).getText();
		...
		if (!alert_unread.equals("0")) {
			if (cmf.existsElement(EndUser.alert_type, driver) == true) {
				...
				test.log(LogStatus.PASS, "Alert Details",
					...
					test.log(LogStatus.FAIL, "Alert Details",
							"Exception occured while fetching Alert Titlte and its Type "+e);
					throw new Exception("Exception occured while fetching Alert Titlte and its Type "+e);
				}
				...
				if (!alert_unread.equals(alter_unread)) {
					...
					Log.error("Alert count is not reduced after reading the recent alert and count is " + alter_unread);
					test.log(LogStatus.FAIL, "Alert Count",
							"Alert count is not reduced after reading the recent alert and count is " + alter_unread);
					Reporter.log("Alert count is not reduced after reading the recent alert and count is " + alter_unread);
					throw new Exception("Alert count is not reduced after reading the recent alert and count is " + alter_unread);
				}
			} else {
