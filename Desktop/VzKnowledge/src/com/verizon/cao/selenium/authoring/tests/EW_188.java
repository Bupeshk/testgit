...
@Test
public class EW_188 extends SetupBase implements Authoring {

	public static ExtentReports ext_report = new ExtentReports(RepConstants.REPORT_PATH+ "\\AuthoringFinancePublishingContentPageTest.html", true,
			DisplayOrder.OLDEST_FIRST);
	private ExtentTest test;
	CommonFunctions common = new CommonFunctions();
	private String  baseurl ;
	public EW_188() throws Exception {
		super();
	}
	//@BeforeSuite
	@Test
	...
	Thread.sleep(1000);
	test.log(LogStatus.PASS, "AuthoringFinancePublishingContentPageTest", "folderAccountSupport clicked successfully done");
	
		
		AuthoringContentPage.chckcontent(driver).click();
		AuthoringDashboardPage.elePublish(driver).click();
		driver.switchTo().alert().accept();
		test.log(LogStatus.PASS, "AuthoringFinancePublishingContentPageTest", "Test case successfully done");
	}
	@BeforeClass
	public  void setUp()throws Exception {
		DOMConfigurator.configure("log4j.xml");
		Log.startTestCase("AuthoringFinancePublishingContentPageTest");
		Log.info("BeforeClass Method Invoked");
		ext_report.config().documentTitle("VzKnowledge Automation");
		ext_report.config().reportName("AuthoringFinancePublishingContentPageTest");
		ext_report.config().reportHeadline("-AuthoringFinancePublishingContentPageTest");
