...
@Test
public class EW_194 extends SetupBase implements Authoring {

	public static ExtentReports ext_report = new ExtentReports(RepConstants.REPORT_PATH+ "\\AuthoringNetworkTechnologyPublishingContentPageTest.html", true,
			DisplayOrder.OLDEST_FIRST);
	private ExtentTest test;
	CommonFunctions common = new CommonFunctions();
	private String  baseurl ;
	public EW_194() throws Exception {
		super();
	}
	//@BeforeSuite
	@Test
	...
	
	{
		...
		Thread.sleep(1000);
		test.log(LogStatus.PASS, "AuthoringNetworkTechnologyPublishingContentPageTest", "folderAccountSupport clicked successfully done");
		
		AuthoringContentPage.chckcontent(driver).click();
		AuthoringDashboardPage.elePublish(driver).click();
		driver.switchTo().alert().accept();
		test.log(LogStatus.PASS, "AuthoringNetworkTechnologyPublishingContentPageTest", "Test case successfully done");
	}
	@BeforeClass
	public  void setUp()throws Exception {
		DOMConfigurator.configure("log4j.xml");
		Log.startTestCase("AuthoringNetworkTechnologyPublishingContentPageTest");
		Log.info("BeforeClass Method Invoked");
		ext_report.config().documentTitle("VzKnowledge Automation");
		ext_report.config().reportName("AuthoringNetworkTechnologyPublishingContentPageTest");
		ext_report.config().reportHeadline("-AuthoringNetworkTechnologyPublishingContentPageTest");
